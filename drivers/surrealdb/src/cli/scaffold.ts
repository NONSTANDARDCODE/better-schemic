// The files `better-schemic init` writes for a fresh SurrealDB project — a connections-only config via the
...
const CONFIG = `import { defineConfig } from "@better-schemic/core/config";
...
//
// Exported as \`betterSchemic\` (a factory; legacy alias \`schemic\` also works): this config also IS
// your app's DB entrypoint — \`import { betterSchemic } from "./better-schemic.config"\` then
// \`await using db = await betterSchemic.connect()\` (typed per connection). The CLI reads the same config.
export const betterSchemic = defineConfig({
  connections: {
    default: surrealConnection({
      ...
      // \`better-schemic check\` replays migrations into a throwaway engine; defaults to an ephemeral in-memory
      // SurrealDB from your local \`surreal\` CLI. Override here, e.g.:
      // check: { engine: "remote", db: { url: "ws://localhost:8000", namespace: "scratch" } },
    }),
  },
});
export const schemic = betterSchemic; // legacy alias (pre-rename scaffolded name)
export default betterSchemic;
`;

const USER_TABLE = `import { defineTable, s, surql } from "@better-schemic/surrealdb";
...
const SEED = `import { defineSeed } from "@better-schemic/surrealdb";
import { User } from "../schema/tables/user";

// The default seed — \`better-schemic seed\` (no arg) runs this \`index.ts\`. Add more named seeds alongside it:
// \`database/seed/01-users.ts\` runs as \`better-schemic seed users\` (the numeric prefix orders \`seed --all\`).
...
/** The dialect-specific files `better-schemic init` writes, keyed by project-relative path. */
export function initScaffold(): Record<string, string> {
  return {
    "better-schemic.config.ts": CONFIG,
...
// --- `better-schemic new <kind> <name>` — per-kind starter modules ----------------------------------------
 ...
 * message. Used by `better-schemic new`; the CLI writes the result under the kind's display folder.
 */
export function scaffoldEntity(kind: string, name: string): string {
  const C = pascalCase(name);
  switch (kind) {
    case "table":
      return `import { defineTable, s, surql } from "@better-schemic/surrealdb";

// A SCHEMAFULL table. Each field is an \`s.*\` builder (a drop-in for Zod's \`z.*\`) that also carries
// its SurrealQL DDL. Add fields, then run \`better-schemic gen\`.
...
`;
    case "relation":
      return `import { defineRelation, s, surql } from "@better-schemic/surrealdb";
...
`;
    case "view":
      return `import { defineView, s, surql } from "@better-schemic/surrealdb";
...
`;
    case "function":
      return `import { defineFunction, s, surql } from "@better-schemic/surrealdb";
...
`;
    case "access":
      return `import { defineAccess, surql } from "@better-schemic/surrealdb";
...
`;
    case "event":
      return `import { defineEvent, surql } from "@better-schemic/surrealdb";
...
`;
    case "analyzer":
      return `import { defineAnalyzer } from "@better-schemic/surrealdb";
...
`;
    ...
    case "field":
      throw new Error(
        `SurrealDB ${kind}s are authored inline on a table, not as their own file — add it to a table (e.g. \`defineTable("…", { … }).index("${name}", [field])\` or \`s.string().$unique()\`). Try \`better-schemic new table <name>\`.`,
      );
    default:
      throw new Error(
